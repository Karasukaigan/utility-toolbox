...
const scriptPath = path.join(__dirname, '..', 'scripts', 'watermark_remove_rgb.py');
...
function removeWatermark(inputPath, outputPath, rValue, gValue, bValue) {
    return new Promise((resolve, reject) => {
        const command = `python ${scriptPath} ${inputPath} ${outputPath} ${rValue} ${gValue} ${bValue}`;
...
router.post('/', upload.array('images'), (req, res) => {
    const rValue = req.body.rValue;
    const gValue = req.body.gValue;
    const bValue = req.body.bValue;
    const inputPath = 'public/image-cache';
    const outputPath = 'public/image-result-cache';
    removeWatermark(inputPath, outputPath, rValue, gValue, bValue)
        .then(result => {
            res.json(result);
        })
        .catch(error => {
